...
- mGBA uses a fixed 80 MiB WebAssembly linear-memory ceiling. A 64 MiB build was rejected by the maximum-cartridge execution gate because ABI 1 temporarily holds the 32 MiB upload while mGBA retains its cartridge copy. Memory growth stays disabled so the core does not add an Emscripten `env` host import to ABI 1. The smoke gate loads and executes a generated 32 MiB GBA cartridge before accepting the build.
...
The mGBA memory ceiling must be reconsidered only if a valid standard-size cartridge cannot pass the execution gate within 80 MiB. Any increase requires a measured failure case and a new fixed qualification target; memory growth must not silently expand the ABI host-import boundary.
